...
__global__ void kernel_sum_CUDA(double* matrix1, double* matrix2, double* matrix3, int m, int n){
    ...
    if (linha < m && coluna < n) {
        ...
        matrix3[linha * n + coluna] = matrix1[linha * n + coluna] + matrix2[linha * n + coluna];  
    }
}
...
c_matrix c_matrix::sum_CUDA(const c_matrix& obj) const{
    ...
    if (m == obj.m && n == obj.n) {
        ...
        int matrix2_size = m * n;
        int matrix3_size = m * n;
        ...
        kernel_sum_CUDA<<<numero_de_blocos, threads_por_bloco>>>(device_matrix1, device_matrix2, device_matrix3, m, n);
